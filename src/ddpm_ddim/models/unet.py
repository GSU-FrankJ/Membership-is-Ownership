...
def build_unet(config_dict: Optional[dict] = None) -> UNetModel:
    # Filter config_dict to only include fields accepted by UNetConfig
    # (image_size is used elsewhere but not needed for UNet architecture)
    allowed_fields = {"in_channels", "out_channels", "channels", "channel_mults", "num_res_blocks", "attention_resolutions", "dropout"}
    filtered_dict = {k: v for k, v in (config_dict or {}).items() if k in allowed_fields}
    config = UNetConfig(**filtered_dict)
