...
% fill the gap - > proposed method/solution
% summary
% contribution
...
% list existing works: core method + weakness.
% similar works, one sentence. 
% our method strength.
...
Clean (no WM) & --- & 6.21 & 0 & --- \\
\addlinespace
WDM & WM ext.\ 99.2\% & 8.47 & +2.26 & ${\sim}1\times$ \\
Zhao$^\dagger$ & Bit acc.\ 99.8\% & 4.83 & +0.71 & ${\sim}1\times$ \\
\addlinespace
...
\midrule
WDM            & \cmark~(8.47)  & \xmark~(9.83)  & \cmark~(12.1) \\
Zhao$^\dagger$ & \cmark~(4.83)  & \xmark~(6.12)  & \xmark~(8.94) \\
\textbf{MiO}   & \cmark~(6.21)  & \cmark~(6.58)  & \cmark~(10.7) \\
\bottomrule
\end{tabular}
\end{table}
...
closed form via Eq., cutting both the
...
%watermark cost生成时间cost，utility 比较生成图片质量。指标：similarity fid等。我们的模型：change = 0
% \begin{table}[t]
...
This work enables IP holders to verify model ownership with 
objective evidence, though robust deployment requires 
pre-registered watermarks, multiple baselines, and principled 
thresholds to mitigate false claims and retrospective selection.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% REFERENCES
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
