...
\begin{tikzpicture}[
  ...
  font=\small,
  line/.style={-Latex, very thick, draw=black},
  thinline/.style={-Latex, thick, draw=black},
  box/.style={draw=black, rounded corners=2pt, align=left, inner sep=6pt, fill=gray!8},
  stepbox/.style={draw=black, rounded corners=2pt, align=left, inner sep=6pt, fill=gray!6, minimum width=58mm},
  outbox/.style={draw=black, rounded corners=2pt, align=left, inner sep=6pt, fill=gray!6, minimum width=52mm},
  diamondbox/.style={draw=black, diamond, aspect=2.2, inner sep=2.5pt, fill=gray!4},
  decision/.style={draw=black, rounded corners=2pt, align=center, inner sep=6pt, fill=gray!6, minimum width=30mm},
...
% Inputs box (left)
% -----------------------------
\node[box, minimum width=46mm] (inputs) {\textbf{Inputs}\vspace{1mm}\par
\begin{tabular}{@{}l@{}}
Owner model $\mathcal{M}_{\text{own}}$\\
Suspect model $\mathcal{M}_{\text{sus}}$\\
Public reference models $\{\mathcal{M}_j\}$\\
Private watermark set $\mathcal{W}_D$\\
\end{tabular}
};
...
% -----------------------------
\node[stepbox, right=20mm of inputs, yshift=26mm] (s1)
{\textbf{1. t-error calculation}\par
Forward noise + single-step \textbf{reconstruction}\par
\hspace*{0mm}\emph{(compute $e_t(x)$ from $x\in\mathcal{W}_D$)}
};
...
% Criteria diamonds
\node[diamondbox, below=9mm of s5, xshift=-18mm] (c1) {Consistency};
\node[diamondbox, right=10mm of c1] (c2) {Separation};
\node[diamondbox, right=10mm of c2] (c3) {Ratio};

\node[notelabel, below=1.5mm of c1] {$\Delta(\text{own},\text{sus})$};
\node[notelabel, below=1.5mm of c2] {$p$-value, Cohen's $d$};
\node[notelabel, below=1.5mm of c3] {error ratio};

% Decision box
\node[decision, below=10mm of c2, minimum width=62mm] (verdict)
{\textbf{Verdict}\par
\textbf{VERIFIED} \quad / \quad \textbf{REJECTED}
};

% -----------------------------
% Output / Evidence report (right, aligned to evidence/decision)
% -----------------------------
\node[outbox, right=18mm of c3, yshift=-2mm] (report)
...
% Criteria to verdict (AND logic)
\draw[thinline] (c1.south) -- ++(0,-5mm) -| (verdict.north);
\draw[thinline] (c2.south) -- (verdict.north);
\draw[thinline] (c3.south) -- ++(0,-5mm) -| (verdict.north);

% Split inputs into two labeled arrows (fix semantics)
\draw[thinline] (inputs.east) -- node[above, notelabel] {models} ($(s1.west)+(0,2mm)$);
\draw[thinline] (inputs.east) -- node[below, notelabel] {$x\in\mathcal{W}_D$} ($(s1.west)+(0,-2mm)$);

% Report connection (from criteria/decision region, not from the middle of pipeline)
\draw[thinline] (c3.east) -- (report.west);
\draw[thinline] (verdict.east) -- ++(8mm,0) |- (report.south);

% Optional: small note for public references usage
\node[notelabel, above=2mm of report] {Compare $\mathcal{M}_{\text{sus}}$ vs $\{\mathcal{M}_j\}$ (population-level forensics)};

\end{tikzpicture}
\vspace{-3mm}
\caption{\textbf{Overview.} We treat the private evidence set $\mathcal{W}_D$ as a watermark. Multi-timestep t-error scoring is robustly aggregated, then a conditional Gaussian model in log-score space yields a closed-form threshold at a target FPR. Ownership is supported via population-level evidence against public reference models using three criteria (consistency, separation, ratio).}
\vspace{-2mm}
\label{fig:overview}
\end{figure*}
