...
\node[stepbox, below=4mm of s1] (s2)
{\textbf{2. Multi-timestep aggregation ($K{=}50$)}\par
$s(x)=Q_{0.25}\!\bigl(\{e_{t_i}(x)\}_{i=1}^K\bigr)$
};

\node[stepbox, below=4mm of s2] (s3)
{\textbf{3. Log-score transformation}\par
$y=\log(1+s(x))$
};

\node[stepbox, below=4mm of s3] (s4)
{\textbf{4. Gaussian QR threshold}\par
Fit $\mu(x),\sigma(x)$; threshold $\tau=\mu+\sigma\Phi^{-1}(1{-}\text{FPR})$
};

\node[stepbox, below=4mm of s4] (s5)
{\textbf{5. Three-point verification (AND)}
};

% ========== DIAMONDS: evenly spaced, centered under s5 ==========
\path (s5.south) ++(0,-10mm) coordinate (diamondrow);
\node[diamondbox] (c2) at (diamondrow) {Separation};
\node[diamondbox, left=20mm of c2] (c1) {Consistency};
\node[diamondbox, right=20mm of c2] (c3) {Ratio};

% Metric labels below each diamond (with enough gap)
\node[notelabel, below=4mm of c1] (lbl1) {$\Delta(\text{own},\text{sus})$};
\node[notelabel, below=4mm of c2] (lbl2) {$p$-value, Cohen's $d$};
\node[notelabel, below=4mm of c3] (lbl3) {error ratio};

% ========== VERDICT: centered below diamonds (extra space for bus line) ==========
\node[decision, below=20mm of c2] (verdict)
{\textbf{Verdict: VERIFIED / REJECTED}\par
(pass threshold AND all three criteria)
};

% ========== RIGHT: Evidence/Report ==========
% Position to the right of the pipeline, aligned vertically with s4/s5
\node[outbox, right=14mm of s4.east, anchor=west, yshift=-8mm] (report)
{\textbf{Evidence Report}\vspace{1mm}\par
\begin{tabular}{@{}l@{}}
$\bullet$ FPR threshold $\tau$\\
$\bullet$ $p$-value vs refs\\
$\bullet$ Cohen's $d$\\
$\bullet$ error ratio\\
\end{tabular}
};

\node[notelabel, above=2mm of report, text width=3.8cm, align=center]
{Population-level forensics:\\$\mathcal{M}_{\text{sus}}$ vs $\{\mathcal{M}_j\}$};
...
% s5 to diamonds: single arrow to c2, then fan out
\draw[thinline] (s5.south) -- (c2.north);

% ========== ARROW: Inputs to s1 (single arrow) ==========
\draw[thinline] (inputs.east) -- (s1.west);

% ========== ARROWS: Diamonds to Verdict via horizontal bus ==========
% Bus line immediately below diamonds (ABOVE labels)
\path (c2.south) ++(0,-2mm) coordinate (bus_y);
\path (bus_y -| c1) coordinate (bus_l);
\path (bus_y -| c2) coordinate (bus_c);
\path (bus_y -| c3) coordinate (bus_r);

% c1: short drop to bus, then horizontal to center
\draw[semithick] (c1.south) -- (bus_l) -- (bus_c);
% c2: short drop to bus center
\draw[semithick] (c2.south) -- (bus_c);
% c3: short drop to bus, then horizontal to center
\draw[semithick] (c3.south) -- (bus_r) -- (bus_c);

% Single arrow from bus center down (past labels) to verdict
\draw[thinline] (bus_c) -- (verdict.north);

% ========== ARROWS: Report connections ==========
% s5 to Report: horizontal arrow
\draw[thinline] (s5.east) -- (report.west);

% Verdict to Report: right then up (one bend, avoids Ratio)
