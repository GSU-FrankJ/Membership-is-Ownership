...
five capability dimensions: whether the method modifies the target
model's weights, the additional training cost it incurs, its FID cost
(the generation-quality penalty attributable to the method),
its per-sample membership AUC, and whether its verification set is
bound by a pre-registered cryptographic commitment.
...
Among the two inference-only methods, MiO is additionally
\emph{hash-committed}: the private watermark set $\mathcal{W}$ is
pre-registered via a SHA-256 commitment, tying any subsequent
verification to a verifiable prior claim (Section~\ref{sec:problem}).
...
MiO).  The gap reflects the structural advantage of the hash-committed
watermark set: when the defender commits a known member subset,
per-sample detection becomes tractable at stringent false-positive
rates, whereas open-ended dataset inference operates in a harder
setting.
...
A separate experiment in Appendix~\ref{app:retroactive} examines
whether an adversary who did not train the model can retroactively
fabricate a passing MiO claim; without access to the hash-committed
watermark set the two verification criteria reliably reject such
attempts.
...
MiO evaluated on its owner DDIM with a 5K hash-committed watermark
set.}
...
\toprule
Method & Modifies weights? & Training overhead & FID cost & Per-sample AUC & Hash-committed \\
\midrule
WDM & \cmark & ${\sim}1\times$ & $+7.21$ & --- & \xmark \\
Zhao$^\dagger$ & \cmark & ${\sim}1\times$ & $+3.07$ & --- & \xmark \\
\addlinespace
CDI & \xmark & $0$ & $0$ & $0.586$ & \xmark \\
\textbf{MiO (Ours)} & \xmark & $\mathbf{0}$ & $\mathbf{0}$ & $\mathbf{0.987}$ & \cmark \\
\bottomrule
