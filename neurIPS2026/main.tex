...
On CIFAR-10/100, STL-10, and CelebA, all datasets achieve $|d| > 18$ and error ratios $> 19\times$ against public baselines; these separations persist after MMD fine-tuning (simulating post-theft adaptation), provided the adversary does not retrain from scratch.
\end{abstract}
...
\begin{itemize}
    ...
    \item We demonstrate $|d| > 18$ and error ratios exceeding
...
base weights via fine-tuning), the raw $t$-error ratio between
owner and reference approaches $1\times$ and
Algorithm~\ref{alg:verification}'s absolute-score criteria
C2/C3 no longer apply directly.  We replace per-sample scores $s(x)$
with paired differences
$\delta(x) = s_{\text{tgt}}(x) - s_{\text{ref}}(x)$ and apply
C1/C2/C3 to the $\delta$ distribution (see
...
Verification requires \emph{every} reference to satisfy $|d|>2.0$ and
ratio${}>5\times$ (ALL-must-pass).
All $12$ (dataset, reference) pairs satisfy both thresholds;
...
\toprule
 & \multicolumn{2}{c}{Semantic-similar DDPM}
 & \multicolumn{2}{c}{Cross-domain LSUN DDPM}
 & \multicolumn{2}{c}{Random init} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
Dataset & $|d|$ & ratio & $|d|$ & ratio & $|d|$ & ratio \\
\midrule
CIFAR-10  & $24.1$  & $24.5\times$  & $40.9$        & $50.3\times$  & $63.9$ & $105.7\times$ \\
CIFAR-100 & $18.5$  & $19.3\times$  & $31.6$        & $42.2\times$  & $63.0$ & $91.1\times$  \\
STL-10    & $116.7$ & $360.2\times$ & $29.3^\dagger$ & $107.1\times$ & $59.3$ & $391.9\times$ \\
CelebA    & $26.3$  & $26.8\times$  & $32.0$        & $49.3\times$  & $90.0$ & $198.4\times$ \\
\bottomrule
\end{tabular}
...
\paragraph{SGD fine-tuning.}
In a separate experiment we fine-tuned Model~A for 500 steps of
plain SGD on an auxiliary CIFAR-10 split, producing a derivative
whose mean $t$-error on $\mathcal{W}$ is $28.71$ (vs.\ $28.73$
for the clean owner).  Scoring this derivative against the public
\texttt{google/ddpm-cifar10-32} reference on $\mathcal{W}$ yields
$\bar{s}_{\text{ref}}=703.84$, a C3 ratio of $24.52\times$
(compared to $24.51\times$ for the clean owner against the same
reference), well above the $5\times$ threshold.
The two remaining references
(\texttt{ddpm-bedroom}, random-initialisation) were not scored for
this derivative because the evaluation job exhausted GPU memory on
the 256-resolution LSUN-bedroom model before completing, and the
owner (Model~A) checkpoint required to re-run the evaluation is no
longer available in our current snapshot.
We therefore cannot report C1 (consistency) or the full C2
(separation) for SGD fine-tuning; the partial evidence is
consistent with the MMD fine-tuning result that light parameter
adaptation leaves the memorization signature on $\mathcal{W}$
essentially unchanged.
The clean-owner baseline numbers ($28.73$, $704.38$) are taken
from the primary evaluation run (Section~\ref{sec:experiments});
the SGD fine-tuning numbers come from a subsequent standalone
evaluation whose full sweep was cut short by GPU memory
constraints.  Stochastic variation between these two runs is
below $0.1\%$ on all quantities reported here.
...
methods (WDM~\citep{peng2023watermark}, which learns a secondary
watermark diffusion process alongside the standard DDPM objective,
and Zhao et al.~\citep{zhao2023recipe}, which embeds a StegaStamp
bit-string into training images before fitting an EDM model) and
one MIA-based dataset-inference method
(CDI~\citep{dubinski2024cdi}), all with publicly available
official implementations.  The two watermarking methods are
trained on the same 50K CIFAR-10 images so that differences in
...
CDI~\citep{dubinski2024cdi} is an inference-time dataset-inference
method that aggregates seven complementary MIAs (denoising loss,
SecMI-stat, PIA, PIAN, gradient masking, noise optimization,
multiple-loss) through a logistic-regression meta-classifier,
producing a single membership score per image.  Unlike the
watermarking baselines, CDI does not embed a signal during training;
it shares MiO's operating regime (no FID penalty, zero training
overhead) and is the closest methodological neighbour to our
approach.  We evaluate CDI on \texttt{google/ddpm-cifar10-32}
using the authors' released codebase and default 7-attack
configuration (5K members + 5K non-members).

The two watermarking baselines (WDM and Zhao) achieve near-perfect
native verification on clean checkpoints, consistent with their
published numbers.  Where the
...
CDI, by contrast, aggregates seven off-the-shelf MIAs against a
public DDPM and reaches AUC${=}0.586$ with TPR${=}0.26\%$ at
FPR${=}0.1\%$; this is close to the random-chance operating point
at stringent FPR budgets and is an order of magnitude below MiO's
$0.987$ AUC / $89\%$ TPR on the same CIFAR-10 task.  The gap
reflects the structural advantage of MiO's hash-committed watermark
set: when the defender commits a known member subset, per-sample
detection becomes tractable at stringent FPRs, whereas open-ended
dataset inference operates in a harder setting.  We note that
CDI and MiO use different base checkpoints (public
\texttt{google/ddpm-cifar10-32} for CDI; our custom DDIM for MiO),
precluding a strict apples-to-apples model-level comparison.  The
gap in AUC and TPR is nonetheless substantial enough
(${>}4\times$ and ${>}300\times$ respectively) that base-model
variance alone cannot explain it; the primary driver is the
methodological distinction between open-ended dataset inference
(CDI) and hash-committed ownership verification (MiO).
...
\begin{table}[t]
\caption{Controlled comparison with watermarking and MIA-based
baselines on CIFAR-10.
Watermarking methods (WDM, Zhao) trained on identical 50K images.
CDI evaluated on \texttt{google/ddpm-cifar10-32} (public DDPM,
5K+5K samples).
...
\addlinespace
CDI$^{\S}$ & TPR 0.26\%@0.1\% & n/a$^{\P}$ & n/a$^{\P}$ & 0.586 & \textbf{0} \\
...
\multicolumn{6}{@{}l}{\footnotesize $^{\S}$CDI native output is a dataset-level $p$-value; we report per-sample TPR/AUC to match MiO's reporting.} \\
\multicolumn{6}{@{}l}{\footnotesize $^{\P}$CDI does not modify the target model; FID not applicable.} \\
\end{tabular}
\end{table}
...
Larger noise magnitudes ($\sigma \geq 10^{-2}$) destroy the model's
reconstruction capability on both member and non-member samples (owner
$t$-error increases ${\sim}2\times$, Criterion~3 ratio ${<}1$),
producing a degenerate regime reflecting model destruction rather than
successful watermark removal; we therefore restrict the reported noise
column to $\sigma = 10^{-3}$.}
...
Experiments across four datasets demonstrate $|d| > 18$ separation (up to $|d| = 33.4$ on STL-10), with criteria satisfied even after adversarial MMD fine-tuning.
...
\begin{equation}
\text{C3}_d: |d| > 3.0 \text{ (vs. current C2: }
|d| > 2.0\text{)}
\end{equation}
...
\paragraph{Motivation.}
On SD paired-delta, LoRA r$=256$ achieves $|d| = 2.55$ (between
...
\paragraph{Empirical observation on SD.}
LoRA r$=64$: $|d| = 2.43$ (below 3.0 threshold);
LoRA r$=256$: $|d| = 2.55$ (below 3.0 threshold).
...
($|d|_{\text{membership}} = 3.93$), this does not translate to stronger
...
\begin{tabular}{@{}lccc@{}}
\toprule
Adversary Strategy & $|d|$ & Ratio & Three-point \\
\midrule
