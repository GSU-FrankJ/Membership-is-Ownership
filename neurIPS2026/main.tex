...
\begin{algorithmic}
\REQUIRE Models $M_A, M_B$, baselines $\{M_{\text{ref}}^{(i)}\}_{i=1}^{|\mathcal{R}|}$, watermark set $\mathcal{W}$
...
\STATE \textbf{O2:} \textbf{for each} reference $i \in \{1, \ldots, |\mathcal{R}|\}$:
\STATE \quad $S_{\text{ref}}^{(i)} \leftarrow \{s_{M_{\text{ref}}^{(i)}}(x) : x \in \mathcal{W}\}$
\STATE \quad $p_2^{(i)}, d^{(i)} \leftarrow \text{TTest}(S_A, S_{\text{ref}}^{(i)}), \text{CohenD}(S_A, S_{\text{ref}}^{(i)})$
\IF{$\exists i: p_2^{(i)} > 10^{-6}$ \textbf{or} $|d^{(i)}| < 2.0$}
...
\begin{table}[t]
\caption{Membership inference performance. DDIM rows use Gaussian QR margins (TPR computed at fixed FPR with 95\% bootstrap CI, 200 iterations). SD rows use latent-space paired-delta scores evaluated on 1{,}000 members + 1{,}000 non-members; TPR at $0.01\%$ FPR is omitted as it falls below the discretization floor (1 false positive at this split size).}
...
\addlinespace
SD v1.4 (LoRA $r\!=\!64$) & $0.995$ & $0.582$ & — \\
SD v1.4 (LoRA $r\!=\!256$) & $0.999$ & $0.879$ & — \\
SD v1.4 (Full FT) & $0.999$ & $0.847$ & — \\
\bottomrule
\end{tabular}
\end{small}
\end{table}

\textbf{Threshold Sensitivity}: The threshold $|d| > 2.0$ is conservative. Table~\ref{tab:threshold_sens} reports the minimum $|d|$ each dataset achieves and the strictest integer threshold it still passes. The smallest margin (CIFAR-100 at $9.3\times$) leaves substantial headroom above the recommended threshold, confirming that results are not artifacts of threshold tuning.

\begin{table}[t]
\caption{Verification margin per dataset. Each row reports the minimum $|d|$ across all three reference models (semantic-similar / cross-domain LSUN / random initialization), the strictest integer $|d|$ threshold that the dataset still passes, and the multiplicative margin above our recommended threshold $|d| > 2.0$.}
\label{tab:threshold_sens}
\centering
\begin{small}
\begin{tabular}{@{}lccc@{}}
\toprule
Dataset & Min $|d|$ achieved & Strictest threshold passed & Margin above $|d|>2.0$ \\
\midrule
CIFAR-10  & 24.1 & $|d| > 24$ & 12.0$\times$ \\
CIFAR-100 & 18.5 & $|d| > 18$ & 9.3$\times$ \\
STL-10    & 29.3 & $|d| > 29$ & 14.7$\times$ \\
CelebA    & 26.3 & $|d| > 26$ & 13.2$\times$ \\
\bottomrule
\end{tabular}
\end{small}
\end{table}
...
\subsubsection{DDIM Owner Model Training}
...
\subsubsection{Stable Diffusion Owner Fine-tuning}

All three configurations share the same 1{,}000-image private subset of COCO 2014 as the watermark set $\mathcal{W}$; no per-configuration data resampling.

We fine-tune Stable Diffusion v1.4 from the official base checkpoint. Common settings: AdamW optimizer, mixed-precision fp16, batch size 4, gradient accumulation 1, $512{\times}512$ resize with center crop, per-image COCO captions used as text conditioning, cosine learning-rate schedule with 500 warmup steps, seed 42.

Variants:
\begin{itemize}
\item \textbf{LoRA $r{=}64$ (A6):} 20{,}000 steps, lr $1{\times}10^{-4}$, rank/$\alpha = 64/64$, target modules \{\texttt{to\_q}, \texttt{to\_k}, \texttt{to\_v}, \texttt{to\_out.0}\}, with random horizontal flip during data loading.
\item \textbf{LoRA $r{=}256$ (A8):} 20{,}000 steps, lr $1{\times}10^{-4}$, rank/$\alpha = 256/256$, same target modules as A6.
\item \textbf{Full FT (A7):} 10{,}000 steps, lr $5{\times}10^{-6}$, all 860M U-Net parameters trainable.
\end{itemize}
...
\subsubsection{WDM Baseline}

Architecture: U-Net with 128 base channels, multipliers $[1,2,2,2]$, 3 residual blocks per stage, attention at resolutions $\{16, 8\}$, 4 attention heads, GroupNorm(32), SiLU activation, dropout 0.1. Noise schedule: linear $\beta$ over $T{=}1000$ (note: differs from MiO's cosine schedule). Prediction: $\epsilon$-prediction. Training: 300{,}000 steps, batch 32, lr $1{\times}10^{-4}$, EMA 0.999, AdamW. Watermark-specific: $\gamma_1 = 0.8$ (training blend factor), $\gamma_2 = 0.1$ (training loss weight on watermarked samples; $\gamma_2 = 0.2$ at extraction), watermark seed 1998, fixed trigger image. Loss: $\mathcal{L} = \mathrm{MSE}(\epsilon) + \gamma_2 \cdot \mathrm{MSE}(\epsilon_{\text{wm}})$. FID sampling uses DDIM 50 steps.

\subsubsection{Zhao Baseline}

Three-stage pipeline. \textbf{Stage 1}: train StegaStamp encoder/decoder at $32{\times}32$ resolution, batch 64, 100 epochs, 64-bit fingerprint. \textbf{Stage 2}: embed a fixed 64-bit fingerprint (seed 42) into all 50K CIFAR-10 training images. \textbf{Stage 3}: train EDM with SongUNet (ddpmpp) backbone, 128 channels, multipliers $[2,2,2]$, 4 blocks per stage, attention at resolution 16; EDMPrecond head with $\sigma$-weighted $\hat{x}_0$-prediction MSE loss; total batch 512, dropout 0.13, augmentation probability 0.12, lr $10^{-3}$, EMA 0.5, seed 42. Training duration was early-stopped at 15 Mimg (the original paper trains to 200 Mimg; full duration would require approximately 57 single-GPU-days). The 15 Mimg checkpoint achieves 100\% fingerprint bit accuracy on watermark detection, sufficient for the baseline comparison. FID sampling uses Heun 18 steps.
