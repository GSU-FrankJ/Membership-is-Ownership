...
When $\mathcal{M}_B$ is a derivative of $\mathcal{M}_A$ (sharing base weights via fine-tuning), the raw $t$-error gap between owner and reference collapses: per-image reconstruction error is dominated by per-image variance shared between the two models, and the unpaired C2 effect size in §\ref{sec:criteria} loses power. We therefore apply C2 to paired differences
\begin{equation}
\delta(x) = s_{\text{tgt}}(x) - s_{\text{ref}}(x),
\label{eq:paired_delta}
\end{equation}
where $s_{\text{tgt}}$ is the target model's score and $s_{\text{ref}}$ is the reference model's score on the same image. Since $s_{\text{ref}}(x) - s_{\text{ref}}(x) \equiv 0$ identically, paired-C2 reduces to a one-sample test of $\delta(\mathcal{W})$ against zero:
\begin{equation}
|d|_{\text{C2}}^{\text{paired}} = \frac{|\,\text{mean}(\delta(\mathcal{W}))\,|}{\text{std}(\delta(\mathcal{W}))}.
\label{eq:paired_c2}
\end{equation}
This is the paired form of the unpaired C2 in §\ref{sec:criteria} and tests the same null hypothesis --- that the target model reconstructs $\mathcal{W}$ no better than the reference --- while cancelling per-image variance shared between the two models. C1 is applied analogously: a one-sample $t$-test on $\delta(\mathcal{W})$ replaces the two-sample test between the owner's and suspect's score distributions. Full treatment of the SD setting using this protocol is in Section~\ref{sec:sd}.
...
Dataset & Semantic-similar DDPM & Cross-domain LSUN DDPM & Random init & Min $|d|$ & Margin \\
        & $|d|$ & $|d|$ & $|d|$ & & \\
\midrule
CIFAR-10  & $24.1$  & $40.9$         & $63.9$ & $24.1$ & $12.0\times$ \\
CIFAR-100 & $18.5$  & $31.6$         & $63.0$ & $18.5$ & $9.3\times$ \\
STL-10    & $116.7$ & $29.3^\dagger$ & $59.3$ & $29.3$ & $14.7\times$ \\
CelebA    & $26.3$  & $32.0$         & $90.0$ & $26.3$ & $13.2\times$ \\
\bottomrule
\end{tabular}
...
C2 ($|d|$): paired-delta effect size $|d|_{\text{C2}}^{\text{paired}} = |\text{mean}(\delta(\mathcal{W})) / \text{std}(\delta(\mathcal{W}))|$ as defined in Equation~\ref{eq:paired_c2}.
...
\paragraph{Stable Diffusion fine-tuning.} All three configurations share the same 1{,}000-image private subset of COCO 2014 as the watermark set $\mathcal{W}$; no per-configuration data resampling. We fine-tune Stable Diffusion v1.4 from the official base checkpoint. Common settings: AdamW optimizer, mixed-precision fp16, batch size 4, gradient accumulation 1, $512{\times}512$ resize with center crop, per-image COCO captions used as text conditioning, cosine learning-rate schedule with 500 warmup steps, seed 42.
...
\subsubsection{Paired-Delta C2 Computation}

The Stable Diffusion verification numbers in Table~\ref{tab:sd_verification} are computed using the paired-delta form of C2 (Equation~\ref{eq:paired_c2}): for each adversary configuration, $\delta(x) = s_{\text{tgt}}(x) - s_{\text{ref}}(x)$ is computed on $\mathcal{W}$ from the per-image score CSVs (one row per image, columns \texttt{score\_tgt} and \texttt{score\_ref}), and $|d|_{\text{C2}}^{\text{paired}} = |\text{mean}(\delta) / \text{std}(\delta)|$ is computed directly. A standalone runner that reproduces the Table~\ref{tab:sd_verification} C2 values from the released CSVs will be released with the camera-ready code.
