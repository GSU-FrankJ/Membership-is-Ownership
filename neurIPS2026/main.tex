...
{\textbf{5. Two-point criteria (AND)}\par
};
...
% -----------------------------
\node[outbox, right=15mm of c2.east, anchor=west] (report)
{\textbf{Evidence / Report}\vspace{1mm}\par
Decision: pass $\tau(\mathrm{FPR})$ \textbf{AND} both criteria hold\par\vspace{1mm}
\begin{tabular}{@{}l@{}}
...
\draw[semithick] (c1.south) -- (c2.south);
...
\paragraph{Auxiliary set construction.}
We construct the auxiliary set $\mathcal{D}_{\text{aux}}$ from the standard
\emph{held-out} split of each dataset, disjoint from the owner's training data.
For DDIM experiments, $\mathcal{D}_{\text{aux}}$ is drawn from the official
test split of CIFAR-10/100, STL-10, and CelebA, while the owner is trained
only on the corresponding train split, of which $\mathcal{W}$ is a $K$-sample
random subset. For Stable Diffusion, $\mathcal{D}_{\text{aux}}$ comprises
1{,}000 COCO~val2014 images, while the owner is fine-tuned on a disjoint
1{,}000-image COCO~train2014 subset that coincides with $\mathcal{W}$.
In both regimes, $\mathcal{D}_{\text{aux}}$ contains genuine non-member samples
for the owner, and the resulting Gaussian QR calibration reflects the null
distribution of reconstruction errors on data the owner has never seen.
...
C2 no longer applies directly.  We replace per-sample scores $s(x)$
...
$\bar{s}_{\text{ref}}=703.84$, yielding a strong error ratio of $24.52\times$
...
the two-point criteria reliably reject such attempts.
...
absolute-score criteria (C2) do not reach their thresholds in raw form.
...
\begin{tabular}{@{}llccl@{}}
\toprule
Owner & Adversaries tested & C1 & C2 ($|d|$) & Verdict \\
\midrule
LoRA $r\!=\!64$ & Domain + Task & \cmark & \cmark~(2.43) & \cmark \\
\midrule
Full FT & Domain + Task & \cmark & \cmark~(2.38) & \cmark \\
\midrule
LoRA $r\!=\!256$ & Domain + Task & \cmark & \cmark~(2.55) & \cmark \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Owner-level membership detection.}
As a sanity check on the underlying signal strength, we evaluate the owner
model alone (independent of any adversary) on $\mathcal{W}$: 1{,}000 watermark
members vs.\ 1{,}000 disjoint COCO non-members. This yields
AUC $= 0.995 / 0.999 / 0.999$ and Cohen's $d = 3.26 / 3.41 / 3.93$
for LoRA $r{=}64$ / Full FT / LoRA $r{=}256$ respectively.
Note this is a \emph{signal-availability} measurement on the owner alone,
distinct from the adversary-specific paired-delta verification reported in
Table~\ref{tab:sd_verification}.

LoRA $r\!=\!64$ and full fine-tuning pass both criteria in the
paired-delta verification protocol.
LoRA $r\!=\!256$ achieves the highest per-sample detection
(AUC${}\!=\!0.999$) and also passes both verification criteria.
The strong separation persists across all three adapter configurations,
establishing that the ownership signal remains robust under realistic post-theft fine-tuning scenarios.
...
Experiments across four datasets demonstrate Cohen's $d > 18$ separation across all reference models, with criteria satisfied even after adversarial MMD fine-tuning.
...
$\mathcal{W}'$ that passes the two-point verification protocol.
Cohen's $d$ is computed against public baselines.
Ratio column is reported for reference only and is no longer a verification criterion.
Two-point: both verification criteria
(Sec.~\ref{sec:criteria}) satisfied (\cmark) or not (\xmark).}
\label{tab:retroactive}
\centering
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Adversary Strategy & Cohen's $d$ & Ratio (ref.) & Two-point \\
\midrule
