...
On CIFAR-10/100, STL-10, and CelebA, all datasets achieve Cohen's $d > 18$ against public baselines; these separations persist after MMD fine-tuning (simulating post-theft adaptation), provided the adversary does not retrain from scratch.
\end{abstract}
...
The resulting separations are striking. Against public baselines, we observe Cohen's $d > 18$, a margin that remains robust even under MMD fine-tuning, which simulates realistic post-theft adaptation, provided the adversary stops short of full retraining or targeted memorization erasure.
...
\begin{itemize}
    ...
    \item We demonstrate that MiO achieves robustness comparable
    to training-time watermarking methods (WDM, Zhao) under
    fine-tuning and noise-perturbation attacks, without modifying
    model weights, incurring training overhead, or degrading
    generation quality.

    \item We demonstrate Cohen's $d > 18$ across four datasets,
    with separations remaining robust under MMD fine-tuning,
    establishing strong empirical guarantees against realistic
    post-theft adaptation.
\end{itemize}
...
(population-level) & Ownership verdict & \cmark & \cmark & (paired-delta) \\
\bottomrule
...
Experiments across four datasets demonstrate Cohen's $d > 18$ separation, with criteria satisfied even after adversarial MMD fine-tuning.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Adversary Strategy & Cohen's $d$ & Ratio & Two-point \\
\midrule
