...
three owner configurations and two adversary types.
...
C2: separation ($|d|>2.0$).
C3: ratio ($>5.0$).
AUC and $|d|_{\text{mem}}$: membership detection performance on $\mathcal{W}$.}
\label{tab:sd_verification}
\centering
\small
\begin{tabular}{@{}llcccccl@{}}
\toprule
Owner & Adversary & C1 & C2 ($|d|$) & C3 (ratio) & AUC & $|d|_{\text{mem}}$ & Verdict \\
\midrule
\multirow{2}{*}{LoRA $r\!=\!64$}
  & Domain-shift & \cmark & \cmark~(2.43) & \cmark~(7.46$\times$) & 0.995 & 3.26 & \cmark \\
  & Task-shift   & \cmark & \cmark~(2.43) & \cmark~(7.46$\times$) & 0.993 & 3.19 & \cmark \\
\midrule
\multirow{2}{*}{Full FT}
  & Domain-shift & \cmark & \cmark~(2.38) & \cmark~(10.1$\times$) & 0.997 & 3.22 & \cmark \\
  & Task-shift   & \cmark & \cmark~(2.38) & \cmark~(10.1$\times$) & 0.995 & 3.13 & \cmark \\
\midrule
\multirow{2}{*}{LoRA $r\!=\!256$}
  & Domain-shift & \cmark & \cmark~(2.55) & \xmark~(3.62$\times$) & 0.999 & 3.93 & \xmark \\
  & Task-shift   & \cmark & \cmark~(2.55) & \xmark~(3.62$\times$) & 0.998 & 3.87 & \xmark \\
\bottomrule
\end{tabular}
\end{table}
...
(population-level) & Ownership verdict & \cmark (ratio 7.46×) & \xmark (ratio 3.62×) & (paired-delta) \\
\bottomrule
...
LoRA $r\!=\!64$ and full fine-tuning pass all three criteria against
both adversary types, with AUC exceeding $0.98$ in every case.
...
Replace C3 with a distribution-free criterion:
\begin{equation}
\text{C3}_\text{AUC}: \text{AUC}(\mathcal{M}_{\text{own}} \text{ vs. }
\text{baseline}) > 0.90
\end{equation}

computed on the score distributions (or paired-delta on SD) via
non-parametric ROC analysis.

\paragraph{Motivation.}
AUC is invariant to distribution shifts and scale compression.
On SD paired-delta, LoRA r$=256$ achieves AUC $= 0.999$, well above
0.90, whereas the ratio-based C3 compresses to $3.62\times$. This
alternative would allow high-capacity adapters while maintaining
separability.
...
LoRA r$=64$ (C3 pass): AUC $= 0.995$;
LoRA r$=256$ (C3 fail): AUC $= 0.999$.
The AUC difference is negligible ($0.999$ vs. $0.995$), indicating that
AUC alone does not capture the population-level compression that C3 detects.
The driver is C3's sensitivity to the ratio shift from $7.46\times$ to
$3.62\times$---a 49\% compression---while per-sample detectability remains
near-optimal.
...
Both would fail a threshold of 3.0, suggesting this alternative is
overly strict for SD paired-delta comparisons. While r$=256$ achieves
stronger per-sample membership detection ($d_{\text{membership}} = 3.93$),
this does not translate to stronger population-level effect size on the
delta distribution, highlighting the qualitative difference between
per-sample and population-level verification criteria.
