...
Address the reviewer's request for ablation studies on reference model selection: how does the choice of baseline affect verification results? Is architecture/data matching required?
...
- **Line 551**: Lists ONE baseline per dataset — gives no sense of sensitivity
- **Discussion line 882**: "for novel domains, general-purpose baselines provide conservative bounds" — unsupported assertion
- **No ablation** on baseline selection anywhere in the paper
- **Code already supports multi-baseline**: `eval_ownership.py` loads all baselines from `configs/baselines_by_dataset.yaml`, computes per-baseline d/ratio/p, and applies conservative criteria (`min(all_d) > 2.0`, `min(all_ratios) > 5.0`)
...
The paper implicitly tests different baseline types across datasets, but never frames it as a baseline sensitivity analysis:

| Dataset | Baseline | Architecture match? | Data distribution match? | Resolution match? | d | Ratio |
|---------|----------|-------------------|------------------------|------------------|---|-------|
...
**Key observations:**
1. **Architecture never matches** — our DDIM UNet (128ch, [1,2,2,2], cosine schedule) differs from all Google/CompVis baselines. Yet all pass with d > 18.
2. **Data match is irrelevant** — CIFAR-10 has 100% overlap (d=23.9); STL-10 has 0% overlap with a bedroom model (d=33.4). Both pass.
3. **Resolution mismatch is handled** — STL-10 and CelebA baselines are resized from 256 down. Still works.
4. **Mismatched baselines give STRONGER separation** (Phase 05 argument)

### What's missing for a proper ablation

We need **multiple baselines evaluated on the SAME dataset** to show sensitivity. The config already registers these:

**CIFAR-10 (3 baselines):**
...
Running these would produce a proper ablation table showing d and ratio for each baseline type on the same dataset.
...
# CIFAR-10: eval all 3 registered baselines
...
# CelebA: eval all 4 registered baselines
python scripts/eval_ownership.py \
  --dataset celeba \
...
| Dataset | Baselines to compute | Time per baseline | Total |
|---------|---------------------|------------------|-------|
...
### Expected output: per-baseline ablation table

| Dataset | Baseline | Role | Architecture | Training Data | d | Ratio | Verified? |
|---------|----------|------|-------------|--------------|---|-------|-----------|
...
**Expected finding**: All baselines pass verification. Mismatched/random baselines give STRONGER separation. The conservative criterion (min d across all baselines) is dominated by the matched baseline, which is the hardest case.
...
**Key findings:**
1. **Architecture matching is NOT required.** None of our baselines share the same architecture as the owner model, yet all achieve d >> 2.0. The verification signal depends on the memorization fingerprint (unique to the owner's training trajectory), not on architectural similarity.
2. **Training data matching is NOT required.** Domain-mismatched baselines yield *stronger* separation because they cannot reconstruct the watermark set at all. An untrained random model provides the ultimate lower bound.
3. **The conservative criterion uses min(d) across ALL baselines**, so the verification threshold is set by the *hardest* (matched) baseline. Adding more baselines can only tighten, never loosen, the verification.
...
All baselines use different architectures from the owner model.
...
\toprule
Baseline & Role & Arch. & Training Data & $|d|$ & Ratio \\
\midrule
...
### Change 2: Add 1-2 sentences to Discussion (after baseline availability paragraph)

```latex
An ablation across matched, mismatched, and untrained baselines
...
the reference and owner models; mismatched baselines yield strictly
stronger separation, and the conservative criterion reports the
minimum $|d|$ across all references.
```

### Change 3: Expand Section 5.1 setup (line 551)

Replace single-baseline description with multi-baseline framing:
```latex
For each dataset we evaluate against multiple public baselines
...
$|d|$ across all baselines; per-baseline breakdowns appear in
Appendix~\ref{app:baseline_ablation}.
```
...
- [ ] **D4**: Update Table 1 to report conservative min(d) instead of single-baseline d? → Recommend YES — makes the multi-baseline protocol explicit
- [ ] **D5**: Merge this with Phase 09 (multi-baseline expansion) of baseline_comparison? → Recommend YES — same experiment, same infrastructure

## 8. Relationship to Other Phases

| Phase | Connection |
|-------|-----------|
| Phase 04 (baseline access) | Phase 04 argues baselines are available; Phase 08 shows the CHOICE doesn't matter |
| Phase 05 (mismatched baseline) | Phase 05 provides the theoretical argument; Phase 08 provides the empirical ablation |
| Phase 09 (baseline_comparison) | Directly overlaps — same multi-baseline eval infrastructure |
...
~2h GPU total. No training. Uses existing `eval_ownership.py` with `--baselines-config configs/baselines_by_dataset.yaml`. The per-baseline breakdown is already computed by the script (lines 557-573). Just needs to be run and results collected.
